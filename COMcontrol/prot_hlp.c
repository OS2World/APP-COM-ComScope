...
ULONG const aul4xBaudTable[] = {50,75,110,150,300,600,1200,2400,4800,7200,
                                 9600,14400,19200,28800,38400,57600,76800,
                                 92160,115200,153600,230400,460800,0};

ULONG const aul12xBaudTable[] = {50,75,110,150,300,600,1200,2400,4800,7200,
                                 9600,14400,19200,28800,38400,57600,76800,
                                 92160,115200,153600,230400,460800,691200,1382400,0};
...
VOID FillHandshakeDlg(HWND hwnd,DCB *pstComDCB,FIFOINF *pstFIFOinfo)
  ...
  if (pstFIFOinfo->wFIFOflags & FIFO_FLG_TYPE_MASK)
    ...
    if (pstFIFOinfo->wFIFOflags & (FIFO_FLG_TYPE_16650 | FIFO_FLG_TYPE_16654))
      {
...
BOOL FillHdwFIFOsetupDlg(HWND hwnd,BYTE byFlags,FIFOINF *pstFIFOinfo)
  ...
  if ((byFlags & F3_HDW_BUFFER_APO) != 0)
    ...
    if ((pstFIFOinfo->wFIFOflags & FIFO_FLG_TYPE_16750) &&
       ((pstFIFOinfo->wFIFOflags & FIFO_FLG_LOW_16750_TRIG) == 0))
      {
    ...
    if (pstFIFOinfo->wFIFOflags & (FIFO_FLG_TYPE_16750 | FIFO_FLG_TYPE_16654))
      usMaxFIFOload = 64;
    else
...
//        CheckButton(hwnd,HWF_TX_MIN,TRUE);
        ...
        if (pstFIFOinfo->wFIFOsize > 32)
          {
          ControlEnable(hwnd,GB_64BYTEFIFOS,FALSE);
          ControlEnable(hwnd,GB_16BYTEFIFOS,FALSE);
...
//        ControlEnable(hwnd,HWF_TXLOADT,FALSE);
    ...
    case HWF_APO:
      if (!Checked(hwnd,HWF_APO))
        ...
        if (Checked(hwnd,HWF_DISABFIFO))
          {
          if (pstFIFOinfo->wFIFOsize < 64)
            CheckButton(hwnd,HWF_14TRIG,TRUE);
          else
            CheckButton(hwnd,HWF_56TRIG,TRUE);
//          CheckButton(hwnd,HWF_TX_MAX,TRUE);
        ...
        if (pstFIFOinfo->wFIFOsize > 32)
          {
          ControlEnable(hwnd,GB_64BYTEFIFOS,TRUE);
          ControlEnable(hwnd,GB_16BYTEFIFOS,TRUE);
...
//        ControlEnable(hwnd,HWF_TXLOADT,TRUE);
      ...
      break;
    case HWF_1TRIG:
      if ((pstFIFOinfo->wFIFOflags & FIFO_FLG_TYPE_16750) == 0)
        break;
      if (Checked(hwnd,HWF_1TRIG))
        {
        CheckButton(hwnd,HWF_1TRIG,FALSE);
        ControlEnable(hwnd,HWF_4TRIG,TRUE);
        ControlEnable(hwnd,HWF_8TRIG,TRUE);
        ControlEnable(hwnd,HWF_14TRIG,TRUE);
        ControlEnable(hwnd,GB_64BYTEFIFOS,TRUE);
