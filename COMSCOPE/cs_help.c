...
BOOL HelpInit(char szLibName[])
  ...
  hini.phtHelpTable = (PHELPTABLE)MAKELONG(COMSCOPE_HELP_TABLE, 0xFFFF);
  ...
  hwndHelpInstance = WinCreateHelpInstance(habAnchorBlock,&hini);
  ...
  */
  ...
  WinSetHook(habAnchorBlock,HMQ_CURRENT,HK_HELP,(PFN)pfnMessageBoxHelpHook,0L);
  return(TRUE);
  }
...
BOOL pfnMessageBoxHelpHook(HAB hab,SHORT sMode,SHORT sTopic,SHORT sSubTopic,PRECTL prclPosition)
  {
#ifdef DEBUG
  MessageBox(HWND_DESKTOP,"CSBETA Help Hook entered");
#endif
